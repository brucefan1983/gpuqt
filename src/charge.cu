...
(
    std::mt19937& generator, int number_of_atoms, real box_length[3],
    int pbc[3], std::vector<real>& x, std::vector<real>& y, 
    std::vector<real>& z, real* potential
)
{
...
void Charge::find_cell_numbers(int pbc[3], real box_length[3])
{
    ...
    if (pbc[1]) Ny = floor(box_length[1] / rc);
    else Ny = 1;
    if (pbc[2]) Nz = floor(box_length[2] / rc);
    else Nz = 1;
    Nxyz = Nx * Ny * Nz;
}
...
(
    int N, int pbc[3], real box_length[3], std::vector<real>& x,
...
static real find_d12_square
(int pbc[3], real box_length[3], real box_length_half[3], real r12[3])
{
    real d12_square = 0.0;
    for (int d = 0; d < 3; ++d)
    {
...
(
    int number_of_atoms, real box_length[3], int pbc[3],
    ...
    for (int ni = 0; ni < Ni; ++ni)
    {
        ...
        for (int i=-ilim; i<ilim+1; ++i)
        {
            ...
            for (int m = 0; m < cell_count[neighbor]; ++m)
            {
                ...
                real r12[3];
                r12[0] = x[n2] - x1; r12[1] = y[n2] - y1; r12[2] = z[n2] - z1;
                real d12_square = find_d12_square
                (pbc, box_length, box_length_half, r12);
                if (d12_square > rc2) continue;
...
void Charge::find_cell_id
(
    real x, real y, real z, real rc,
    int& nx, int& ny, int& nz, int& nxyz
)
{
    nx = floor(x / rc);
    ny = floor(y / rc);
...
int Charge::find_neighbor_cell
(int nx, int ny, int nz, int nxyz, int i, int j, int k)
{
    int neighbor = nxyz + k * Nx * Ny + j * Nx + i;
    if (nx + i < 0)   neighbor += Nx;
    if (nx + i >= Nx) neighbor -= Nx;
    if (ny + j < 0)   neighbor += Ny * Nx;
    if (ny + j >= Ny) neighbor -= Ny * Nx;
    if (nz + k < 0)   neighbor += Nxyz;
    if (nz + k >= Nz) neighbor -= Nxyz;
    return neighbor;
}
