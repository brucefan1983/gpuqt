...
(
    int N, int pbc[3], double box_length[3], std::vector<double>& x,
...
static double find_d12_square
(int pbc[3], double box_length[3], double box_length_half[3], double r12[3])
{
    ...
    for (int ni = 0; ni < Ni; ++ni)
    {
        ...
        for (int i=-ilim; i<ilim+1; ++i)
        {
            ...
            for (int m = 0; m < cell_count[neighbor]; ++m)
            {
                ...
                double d12_square = find_d12_square
...
int Charge::find_cell_id(double x, double y, double z, double rc)
{
    int nx = floor(x / rc);
    int ny = floor(y / rc);
    int nz = floor(z / rc);
...
(
    double x, double y, double z, double rc,
